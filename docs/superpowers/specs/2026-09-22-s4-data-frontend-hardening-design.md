...
- 状态：已落地除 D1/D5（6 库合一与其版本轨）外的全部条目，见 §8
- 日期：2026-09-22
...
已落地（244 项测试全绿）：F1 窄订阅 + 事件归约 + 流式按帧合并、F2、F3 日志封顶与虚拟化、
F4 配置编辑入撤销栈并按字段合并、F5 导入前校验、A1/A2/A3 键盘可达，
以及 D2（删项目连带清历史 + 启动期清理既有孤儿）、D3（切默认进事务）、D4（统一 pragma 与 busy_timeout）。

**D1「6 库合一」本轮不做，理由记录在此而不是删掉条目：**

1. 合一的动机原本是"外键级联 + 跨库事务"，但两件真正伤人的缺陷（孤儿历史、默认模型切换
   不原子）都已经用代码修掉了：删子再删父的顺序 + 启动期清理，效果对用户等价，
   而 SQLite 的外键也无法跨文件，所以它本来就不是合一才能拿到东西。
2. 剩下能拿到的只有运维层面的好处：一个文件、一条版本轨、一份 journal。
3. 代价是给每一个存量安装引入一次数据迁移，而本仓库**没有真库测试能力**
   （better-sqlite3 按 Electron ABI 构建，纯 Node 下加载即抛 NODE_MODULE_VERSION 不匹配）。
   也就是说，这次迁移的验证手段只有"纯函数 + 人工升级一次"，而它出错的后果是丢用户数据。
4. 下一步是签名、自动更新与崩溃恢复（S5）。在临近"可分发"的位置塞一个不可测的数据迁移，
   顺序是错的。

因此本轮把 D5（六张表的列变动纳入版本轨）也一并推迟：它应当和合一同时做，否则只是给
六个文件各加一套版本记录。

**若日后要做，前置条件是：**先让测试能真正打开一个 SQLite 文件（例如用 Electron 的 node
跑一个迁移冒烟脚本），迁移必须是 ATTACH + 列名交集 INSERT + 行数比对不符即回滚，
遗留文件改名保留而非删除 —— 备份与拒绝启动的骨架 S0 已经有了。

## 9. 一处行尾事故（记下来免得再犯）

`storage/` 六个文件在一次批量改写里混进了 `\r\r\n`，把 20 行的改动放大成全文件重排。
仓库 `core.autocrlf=true`（blob 存 LF，工作区 CRLF），用脚本改文件时应统一按 LF 写、
交给 git 转换；改完必须用 `git diff --stat` 确认改动规模与预期一致，而不是只看测试是否通过。
