...
- **本地数据仍是六个 SQLite 文件**：备份要么整目录拷 `%APPDATA%/ai-workflow`，要么用应用内导出；单文件合一被推迟，理由见规格文档 §8。
...
以前删项目只删项目行，执行历史留在库里：日志页继续列出这些运行，点开却是打不开的空记录。
现在删除会先清历史再清项目，启动时也会把既往积累的孤儿记录清掉（会在日志里报出条数）。

六个库分文件存放，SQLite 做不了跨文件外键，所以这个"级联"是代码层面的顺序保证：
先删子记录再删父记录，中途失败最多留下"历史已清、项目还在"，重试即可；反过来才会留下永久孤儿。

### 数据库连接的统一

六个库过去各自打开连接且只设了 `journal_mode`：没有 `busy_timeout`（两个窗口撞上同一张表
直接抛 `SQLITE_BUSY`），`foreign_keys` 也仍是 SQLite 的默认关闭值。现在统一走一个
`openStore()`，pragma 只有一份。模型库切换默认模型的两条 UPDATE 也包进了事务
（此前中途失败会留下 0 个或 2 个默认模型）。

> 6 库合一到单文件暂未做，原因与前置条件见 `docs/superpowers/specs/2026-09-22-s4-data-frontend-hardening-design.md` §8。
### 配置编辑现在可撤销
